...
private:
    ...
    void printBall( const pos_t & ball )
      {
          std::cout << "<Ball>\n";
    ...
    static
    short toShort( short value )
      {
          return (short)ntohs( value );
      }

    static
    double toDoubleOld( short value )
      {
    ...
    void doHandleShowInfo( std::streampos pos,
    ...
    void parseParams( const char * buf )
      {
          int n_read = 0;
          char name[128];
          char value[512];
          ...
          while ( *buf == '(' )
          {
              if ( std::sscanf( buf, " ( %s %[^)] ) %n ",
                                name, value, &n_read ) != 2 )
              {
                  std::cerr << "Illegal server_param line. \"" << buf << "\"" << std::endl;;
                  break;
              }
              buf += n_read;
