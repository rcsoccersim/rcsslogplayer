...
# This file need to be inside this repository, to copy all lib dependencies: https://github.com/rcsoccersim/rcsslogplayer
# Build command: docker build . -t "simulator2d_rcsslogplayer:core"
# Run command: docker run -it --rm -e DISPLAY=${DISPLAY} -e QT_X11_NO_MITSHM=1 -v /tmp/.X11-unix:/tmp/.X11-unix --name=logplayer --net mynet simulator2d_rcsslogplayer:core
...
WORKDIR /root/
RUN mkdir /rcsslogplayer
COPY . /root/rcsslogplayer
WORKDIR /root/rcsslogplayer
RUN ./bootstrap
RUN ./configure 
RUN make
RUN make install
