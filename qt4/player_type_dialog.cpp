...
protected:
    ...
    bool lessThan( const QModelIndex & left,
                   const QModelIndex & right ) const
      {
          ...
          double lhs = 0.0;
          double rhs = 0.0;
          std::sscanf( src->data( left ).toString().toLatin1(), " %lf ", &lhs );
          std::sscanf( src->data( right ).toString().toLatin1(), " %lf ", &rhs );
...
PlayerTypeDialog::updateData()
{
    ...
    for ( int row = 0; row < ROW_SIZE; ++row )
    {
        const std::map< int , rcss::rcg::PlayerTypeT >::const_iterator it
            = M_main_data.dispHolder().playerTypes().find( row );
        if ( it == M_main_data.dispHolder().playerTypes().end() )
        {
            continue;
        }
