...
void
Parser::convert( const dispinfo_t & from,
                 DispInfoT & to )
{
...
void
Parser::convert( const showinfo_t & from,
                 ShowInfoT & to )
{
...
void
Parser::convert( const short_showinfo_t2 & from,
                 ShowInfoT & to )
{
...
void
Parser::convert( const team_t & from,
                 TeamT & to )
{
...
void
Parser::convert( const player_type_t & from,
                 PlayerTypeT & to )
{
...
void
Parser::convert( const player_params_t & from,
                 PlayerParamT & to )
{
...
void
Parser::convert( const server_params_t & from,
                 ServerParamT & to )
{
    double tmp = 0.0;
    ...
    to.wind_angle_ = nltohd( from.wind_ang );
    to.wind_rand_ = nltohd( from.wind_rand );

    //to.kickable_area_ = nltohd( from.kickable_area );

    to.catchable_area_l_ = nltohd( from.catch_area_l );
    to.catchable_area_w_ = nltohd( from.catch_area_w );
    ...
    to.offside_active_area_size_ = nltohd( from.offside_active_area );

    to.wind_none_ = nstohb( from.wind_none );
    to.wind_random_ = nstohb( from.use_wind_random );
    ...
    to.keepaway_length_ = nltohd( from.ka_length );
    to.keepaway_width_ = nltohd( from.ka_width );
